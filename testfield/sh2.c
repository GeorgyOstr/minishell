...
// #define REDIRECT_P(cmd) ((cmd)->argc == -1)
#define HEAD_P(cmd) ((cmd) == cmdhead) /* 今の cmd が先頭コマンドか？ */
...
parse_command_line(char *p)
{
	...
	while (*p)
	{
		...
		// コマンドラインから” |>”を抽出cmd->argvに格納メモリ領域を必要に応じて拡張しつつ
...
struct builtin			builtins_list[] = {{"cd", builtin_cd}, {"pwd",
				builtin_pwd}, {"exit", builtin_exit}, {NULL, NULL}};
